...
FROM (
SELECT DISTINCT b.facid, b.memid, f.name -- no duplicates
FROM Bookings AS b
LEFT JOIN Facilities AS f ON b.facid = f.facid -- join Bookings and Facilities
WHERE b.facid = 0 OR b.facid = 1 -- tennis court facids
) AS bf
LEFT JOIN Members AS m ON bf.memid = m.memid -- join bf and Members
ORDER BY m.surname, m.firstname; -- order by name
...
GROUP BY sub.facid
HAVING revenue <= 1000; -- (having, not where)
